...
import CreateOrUpdateEmployee from './Modals/CreateOrUpdateEmployee';
...
const EmployeesPage = () => {
    ...
    const getCustomers = async () => {
        try {
            const res = await Get<{ status: string, data: any }>(`/v1/client-user?page=${currentPage}&per_page=10&search=${debouncedSearch ?? ''}${startDate && `&start_date=${startDate}`}${endDate && `&end_date=${endDate}`}&order_by_role=employee`);
            ...
            <div className='space-y-4'>
...

                {
                    modalType === 'add' || modalType === 'edit' ?
                        <CreateOrUpdateEmployee modalType={modalType} closeModal={() => {
                            setModalType(null)
                            setData(null)
