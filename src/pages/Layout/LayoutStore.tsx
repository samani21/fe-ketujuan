    ...
    return (
        <div className="min-h-screen bg-slate-50 font-sans text-slate-900">
                ...
                <div className="max-w-screen-md mx-auto px-4 py-4">
                    <div className="flex items-center justify-between mb-4">
                        <div className="flex items-center gap-3">
                            <div className="w-10 h-10 bg-[#064e3b] rounded-full flex items-center justify-center text-white font-bold text-[8px] text-center leading-tight p-1 shadow-inner">
                                PUREEATS
                            </div>
                            <div>
                                <h1 className="text-sm font-bold text-slate-800 leading-none">PUREEATS</h1>
                                <p className="text-[10px] text-slate-500 mt-1 flex items-center gap-1">
                                    <MapPin size={10} className="text-emerald-700" /> Lokasi Anda: Banjarmasin
                                </p>
                            </div>
                        </div>
                    ...
                    {setSearchQuery && <div className="relative group">
                        <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-emerald-800 transition-colors" size={18} />
                        <input
                            type="text"
                            placeholder="Cari outlet terdekat..."
                            className="w-full bg-slate-100 border border-transparent rounded-xl py-3 pl-10 pr-4 text-sm focus:ring-1 focus:ring-emerald-800 focus:bg-white focus:border-emerald-800 transition-all outline-none"
                            value={searchQuery}
            ...
            <nav className="fixed bottom-0 left-0 right-0 bg-white/80 backdrop-blur-md border-t border-slate-200 px-8 py-3 max-w-screen-md mx-auto flex justify-between items-center shadow-[0_-4px_12px_rgba(0,0,0,0.05)] z-20">
                {
                    listMenu?.map((ls, i) => (
                        ls?.url === pathname ? <div className="flex flex-col items-center gap-1 text-emerald-900 relative cursor-pointer" key={i}>
                            {ls?.icon}
                            <span className="text-[10px] font-black uppercase tracking-tighter">{ls?.name}</span>
                            <div className="absolute -top-1 right-0 w-1.5 h-1.5 bg-emerald-500 rounded-full"></div>
                        </div> :
                            <div className="flex flex-col items-center gap-1 text-slate-400 hover:text-emerald-700 transition-colors cursor-pointer" key={i} onClick={() => route.push(ls?.url)}>
                                {ls?.icon}
